<template>
  <div>
    <div class="content">
      <top-bar />
      <h2 style="margin-top: 20px;">Check In/Out: {{ date }}</h2>
      <PageHeader
        pageCategory="Youth Headers"
        pageName="Check In and Out"
      ></PageHeader>
      <br />
      ...
      <div v-if="userSelected && !isCheckedIn">
        <b-button class="check-in-out-button" @click="checkIn()">
          <h4>
            Check In
          </h4>
          {{ time }}
      ...
      <!-- Check Out -->
      <div v-if="userSelected && isCheckedIn">
        <b-button class="check-in-out-button" @click="showCheckoutModal">
          <h4>
            Check Out
          </h4>
          {{ time }}
      ...
      <b-modal hide-footer v-model="checkoutModalVisible">
        ...
        <b-button
          ...
          >Cancel</b-button
        >
        <b-button class="mt-3" block @click="handleCheckOut()" variant="success"
          >Confirm for staff review</b-button
        >
      </b-modal>
      <b-modal id="checkout-status-modal" hide-footer>
        ...
        <div v-else>
          ...
          <b-button
            class="mt-3"
            block
            @click="$bvModal.hide('checkout-status-modal')"
            >Done</b-button
          >
        </div>
      </b-modal>
...
export default {
  ...
  methods: {
    ...
    async checkOut() {
      ...
      val["Period"] = period;
      val["Check In"] = Timestamp.fromDate(
        moment(this.checkedInUsers[this.ID]["Check In Time"]).toDate()
      );
      val["Check Out"] = Timestamp.fromDate(moment().toDate());

      val = { ...val, ...this.hours };
      let status = await db
        .collection("GlobalPendingHours")
        .doc()
        .set(val);
      if (status) {
      ...
      let newPendingHours =
        Math.round(
          (parseFloat(profile["Pending Hours"]) + categoryHourSum) * 100
        ) / 100;
      ...
      this.checkoutStatus.title = "Success";
      this.checkoutStatus.message =
        "You have successfully checked out. Your hours have been requested.";
