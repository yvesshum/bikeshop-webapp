      ...
      <b-container fluid>
        ...
        <b-row style="width: 100%">
          <PageHeader
            ...
            pageName="Transfer Hours Between Youth"
          ></PageHeader>
        </b-row>
        <b-row style="width: 100%">
          <b-col>
  ...
  methods: {
    ...
    async submit() {
      ...
      if (fromID == null || toID == null) {
        this.showModal(
          "Error",
          "Please select/enter an ID for both 'To' and 'From'"
        );
        return null;
      } else if (fromID === toID) {
        ...
        let fromYouthProfile = await db
          .collection("GlobalYouthProfile")
          .doc(fromID)
          .get();
        let toYouthProfile = await db
          .collection("GlobalYouthProfile")
          .doc(toID)
          .get();
        ...
        let currentHours =
          parseFloat(fromYouthProfile["Hours Earned"]) -
          parseFloat(fromYouthProfile["Hours Spent"]);
        if (currentHours >= amount) {
          ...
          let request_status = await db
            .collection("GlobalTransferHours")
            .doc()
            .set({
              "From ID": fromID,
              "To ID": toID,
              "From Name":
                fromYouthProfile["First Name"] +
                " " +
                fromYouthProfile["Last Name"],
              "To Name":
                toYouthProfile["First Name"] +
                " " +
                toYouthProfile["Last Name"],
              Amount: amount,
          ...
          let newToPendingHours =
            parseFloat(toYouthProfile["Pending Hours"]) + amount;
          let newFromPendingHours =
            parseFloat(fromYouthProfile["Pending Hours"]) - amount;
          let newFromHoursSpent =
            parseFloat(fromYouthProfile["Hours Spent"]) + amount;

          let status1 = await db
            .collection("GlobalYouthProfile")
            .doc(toID)
            .update({
          ...
          this.showModal(
            "Success!",
            "Your request has been sent for staff approval"
          );
        } else {
          this.closeLoadingModal();
          this.showModal(
            "Error",
            "The sender does not have enough hours! They only have " +
