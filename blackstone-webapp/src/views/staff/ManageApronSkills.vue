  ...
  <div class="StaffManageSkills">
    <div class="content">
      ...
      <div>
        ...
        <b-button
          variant="success"
          @click="showRenameCategoryModal"
          style="margin: 1%;"
          >Rename Category</b-button
        >
        <b-button
          variant="danger"
      ...
      <b-modal v-model="addCategoryModalVisible" hide-footer lazy>
        ...
        <div class="centerModal">
          ...
          <SpecialInput
            ...
            ref="new_category"
            inputType="String"
            :arguments="{ ...args.specialInput }"
          ></SpecialInput>
          <h4 class="error_message">{{ errorMsg }}</h4>
          <b-button
            class="mt-3"
            block
            @click="checkError(new_category, true, false)"
            variant="success"
            >Add Category</b-button
          >
          <b-button
      ...
      <b-modal v-model="addSkillModalVisible" hide-footer lazy>
        ...
        <div class="centerModal">
          ...
          <b-button
            class="mt-3"
            block
            @click="checkError(new_skill, false, false)"
            variant="success"
            >Add Skill</b-button
          >
          <b-button
      ...
      <b-modal v-model="renameCategoryModalVisible" hide-footer lazy>
        ...
        <div class="centerModal">
          ...
          <SpecialInput
            v-model="renamed_category"
            ref="renamed_category"
            inputType="String"
            :arguments="{ ...args.specialInput }"
          ></SpecialInput>
          <h4 class="error_message">{{ errorMsg }}</h4>
          <b-button
            class="mt-3"
            block
            @click="checkError(renamed_category, true, true)"
            variant="success"
            >Rename Category</b-button
          >
          <b-button
            class="mt-3"
            block
            @click="closeRenameCategoryModal()"
            variant="secondary"
            >Cancel</b-button
          >
        </div>
      ...
      <b-modal v-model="modalVisible" hide-footer lazy>
        ...
        <b-button class="mt-3" block @click="closeModal" variant="primary"
          >ok</b-button
        >
      </b-modal>
  ...
  methods: {
    ...
    checkError(input, isCat, isRenameCat) {
      if (isCat && input.includes("/")) {
        this.errorMsg =
          "Error: Categories cannot contain the forward slash (/) character.";
      } else if (
        (isCat && input == ".") ||
        input == ".." ||
        input == "Select Category to rename" ||
        input == "Select a category for your skill"
      ) {
    ...
    async submit() {
        ...
        for (const change_string in changes) {
          if (changes[change_string] != null) {
            ...
            if (changes[change_string] == false) {
              if (
                data["Apron Skills Alt"] != undefined &&
                data["Apron Skills Alt"][old_color] != undefined &&
      ...
      if (check) {
        ...
        this.showModal(
          "Error",
          "Unable to submit apron skills, this may be an internet connection problem"
        );
      }
    },
    async getColors() {
    ...
    async getNewData() {
      ...
      this.showModal(
        "Table Refreshed",
        "The table should contain the latest information"
      );
    },
    ...
    async delete_category() {
      for (var i = 0; i < this.categories.length; i++) {
        if (this.categories[i] == this.selected_category) {
          ...
          let new_groups = this.groups.filter(
            group => group.category != this.selected_category
          );
    ...
    async add_skill() {
      for (var i = 0; i < this.groups.length; i++) {
        if (
          this.groups[i].color == this.selected_color.name &&
