<template>
  <div>
    <div class="content">
      ...
      <p style="margin: 0 1rem; padding-top: 0.5rem;">
        Note: This changes Hours Earned and does not show up as a record
      </p>
      <PageHeader
        pageCategory="Staff Headers"
        pageName="Add and Subtract Hours for Multiple Youths"
      ></PageHeader>
      <br />
      <b-container fluid>
        ...
        <b-row style="width: 100%">
          <b-col>
            <b-button variant="success" @click="addButtonClicked"
              >Add Hours</b-button
            >
          </b-col>
          <b-col>
            <b-button variant="danger" @click="subtractButtonClicked"
              >Subtract Hours</b-button
            >
          </b-col>
        </b-row>
      </b-container>
...
export default {
  ...
  methods: {
    ...
    async addButtonClicked() {
      ...
      let newHoursEarned =
        Math.round((parseFloat(profile["Hours Earned"]) + this.value) * 100) /
        100;
      try {
        await db
          .collection("GlobalYouthProfile")
      ...
      this.showModal(
        "Success!",
        profile["First Name"] +
          "'s Hours Earned Increased from " +
          profile["Hours Earned"] +
          " to " +
          newHoursEarned
      );
    },
    ...
    async subtractButtonClicked() {
      ...
      if (this.id == "") {
        ...
        this.showModal(
          "No ID selected",
          "Please select a Youth to subtract hours"
        );
        return;
      }
      let profile = await db
