  ...
  <div v-if="ready">
    <b-card class="post_card">
      <span
        ...
        >Delete</span
      >
      <div class="cardContainer" @click="viewBlogPost(PostObj.id)">
    ...
    <b-modal v-model="modal.conf.visible" hide-footer lazy>
      ...
      <div class="d-block text-center">
        <h3>
          Are you sure you want to delete post with the title: {{ Post.title }}?
        </h3>
      </div>
      <b-button class="mt-3" block @click="closeConfModal" variant="success"
        >Cancel</b-button
      >
      <b-button class="mt-3" block @click="handlePostDelete" variant="danger"
        >Yes, Delete</b-button
      >
    </b-modal>
