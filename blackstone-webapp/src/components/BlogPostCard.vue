<template>
    <div>
            ...
            <div class="container" @click="viewBlogPost(Post.id)">
                <b-row style="margin: 1.25rem 0; width: 100%;">
                    ...
                    <b-col style="margin-left: 1rem; text-align: left;" cols="9">
                        <b-card-title style="padding-right: 1.5rem">{{Post.title}}</b-card-title>
                        <b-card-sub-title style="margin-bottom: 8px;">
                            <i>By {{Post.posterName}}</i>
                        </b-card-sub-title>
                        <b-card-sub-title>Posted Aug 15, 2020</b-card-sub-title>
                    </b-col>
                </b-row>
            </div>
        </b-card>
        ...
        <!-- Post modal -->
        <b-modal title="Title" v-model="showPostModal" size="xl" hide-footer hide-header>
            <BlogPostContent :post="Post" :closeHandler="closePostModal" />
            <b-button id="editButton" variant="info" @click="onEditClicked">
